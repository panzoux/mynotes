...
| `追加 #AI #ツール Gitのrebaseとは` | `#` 以降をタグとして追加。自動タグと併用 |
...
1. コマンドに `#タグ名` が含まれる場合、それを手動タグとして必ず付与する（自動タグと重複する場合は1つにまとめる）
2. テーマを調査し、**わかりやすいHTML記事**を生成する
   - 見出し・本文・コード例・図解（SVGまたは `<figure><img>`）を適宜使う
   - 有用な画像URL（公式ドキュメント・Wikimedia等、信頼性の高いもの）があれば `<figure>` で挿入
3. タグを **2〜4個** 自動付与（内容から判断）し、手動タグと合わせる
4. `notes/` 以下に英語スラッグ名の `.html` を作成
5. `index.html` のリスト先頭に追記（`data-tags` 属性にタグをカンマ区切りで記載）
6. `git add` → `git commit` まで実行し、差分確認を促す（**push しない**）
